    ...
    <div>
      ...
      <div className="card">
        ...
        <p className="muted" style={{ marginTop: -8, marginBottom: 12 }}>
          Post a comment or announcement for the whole team to see - no points, no sign-up.
        </p>
        <form onSubmit={handleCreateAnnouncement} style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'flex-end', marginBottom: 16 }}>
          ...
          <div className="field" style={{ flex: '2 1 220px' }}>
            <label>Message</label>
            <input value={newAnnouncement.message} onChange={(e) => setNewAnnouncement({ ...newAnnouncement, message: e.target.value })} required />
          </div>
          <button className="btn btn-primary" type="submit">Post</button>
        </form>
        {announcements.length === 0 && <p className="muted">No announcements yet.</p>}
        {announcements.map((a) => <AnnouncementCard announcement={a} key={a.id} />)}
      </div>
